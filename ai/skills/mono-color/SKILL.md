---
name: mono-color
description: Generate original one-ink or controlled two-ink editorial raster artwork and production prompts from a theme, phrase, object, article idea, or reference image. Use for 单色海报、双色印刷、单色调视觉、孔版印刷、risograph、网点照片、zine posters, monochrome editorial posters, duotone prints, or explicit mono-color styling. In web/UI work, use only for art direction and raster assets while `impeccable` owns the semantic interface. Generate the image when a native image tool is available; otherwise return the final prompt and recipe.
...
2. one generated raster image when the current harness exposes native image generation;
3. a short recipe note naming palette, layout family, type pairing, and print process.

When image generation is unavailable, return the final prompt and recipe and state the limitation. Do not ask the user to install an external image service merely to satisfy the default deliverable.

Follow the active image-generation tool's save policy. An explicit user destination takes precedence. For project-bound work, move or copy the selected image into an existing project asset directory and report that path. Preview-only output may remain at the provider's default location and render inline. Never claim a file path that does not exist.

## Interface Composition Boundary

The print constraints remain strict for standalone artwork and every generated raster asset.

When the requested deliverable is a website, landing page, app screen, or other interface, compose with `impeccable`. `impeccable` owns information architecture, semantic copy, navigation, CTAs, controls, accessibility, responsive behavior, and implementation. `mono-color` owns the art direction and raster assets such as heroes, textures, and decorative marks.

Apply the Hard Avoids only to generated artwork in this mode; do not use them to remove necessary interface affordances. Keep UI text and controls out of the bitmap and implement them semantically.
...
For standalone artwork and every generated raster asset, always exclude:
