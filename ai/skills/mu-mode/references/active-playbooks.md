...
| Bug Fix | Reproduce a reported defect, establish its failure mechanism, fix the root cause, and verify the same surface. | Fix Root Causes, Prove It Works, Boundary Discipline | Planned: `playbooks/bug-fix.md` |
...
| Feature | Deliver new or changed behavior from an explicit data and interaction shape. | Model the Domain, Experience First, Prove It Works | Planned: `playbooks/feature.md` |
...
| Code Review | Return prioritized, evidence-backed findings; add operational or interactive review capabilities only when warranted. | Minimize Reader Load, Boundary Discipline, Prove It Works | Planned: `playbooks/code-review.md` |
...
- **Investigation vs Figure It Out:** choose Investigation for a bounded read-only question; choose Figure It Out when the workflow itself must be designed.
