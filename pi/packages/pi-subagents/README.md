...
In this dotfiles fork, the `researcher` builtin uses local `pi-parallel` (`web_search`, `web_fetch`, `deep_research`, and `batch_enrich`) with Turbo as the first-pass search lane. Local `pi-exa` provides `exa_search` for semantic/code discovery, broader multilingual search, and fallback verification. Exa requires `EXA_API_KEY`; Parallel requires `parallel-cli` plus `parallel-cli login`; see `pi/README.md` in this repo.
