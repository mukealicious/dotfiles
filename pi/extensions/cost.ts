 ...
 *
 * Parses Pi session JSONL logs to report API spending. Shows total cost,
 * daily breakdown with ASCII bar charts, top 10 projects by spend, and
 * per-model distribution.
 *
 * Usage: /cost [days]  (default: 7)
 *
 * Zero runtime cost — pure log parsing, no API calls.
 *
    ...
    handler: async (args, ctx) => {
      ...
      if (dates.length > 0) {
        ...
        for (const d of dates) {
          ...
          lines.push(
            `   ${d}  ${formatCost(byDate[d]).padStart(8)}  ${bar}`,
          );
        }
        lines.push("");
      }
      ...
      if (projects.length > 0) {
        ...
        for (const [name, cost] of projects) {
          lines.push(
            `   ${name.padEnd(30)} ${formatCost(cost).padStart(8)}`,
          );
        }
        lines.push("");
      }
